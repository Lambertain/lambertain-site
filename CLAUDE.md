...
Тело: `{ "projectKey": "HH", "title": "...", "description": "...", "assigneeLogin"?: "...", "internal"?: false, "triage"?: true, "recipient"?: "client" }`.
`recipient:"client"` — **вопрос/задача клиенту** (не назначается разработчику, без триажа, клиент видит и получает пуш, отвечает комментами). Для уточняющих вопросов клиенту по спеке — использовать именно его.
