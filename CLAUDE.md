...
- `POST /api/admin/project/accounts` `{ projectKey, env:"prod"|"dev", accounts:[{login?,pass?,note?}], mode?:"append"|"replace" }` → аккаунты входа проекта (`meta.prodAccounts`/`devAccounts`, видны разрабу в `/api/dev/secrets`). Разработчик свои dev URL+аккаунты ставит сам через `POST /api/dev/secrets` (project-token).
