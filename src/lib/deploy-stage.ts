...
// Текст-пояснение клиенту при достижении стадии (укр., простыми словами).
// DEV-12: промежуточные стадии ('pr' готується / 'dev' на тестовому) клиенту НЕ шлём (это шум) — только ключевая 'prod'.
// Стадия всё равно трекается (DeployBadge у задачи обновляется), просто без пуша/коммента клиенту.
