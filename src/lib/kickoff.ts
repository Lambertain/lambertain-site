      ...
      tasks: {
        ...
        items: {
          ...
          properties: {
            ...
            client_verifiable: { type: "boolean", description: "Может ли НЕтехнический клиент САМ принять результат глазами/руками. true — задача даёт НОВЫЙ наблюдаемый результат (экран, страница, форма, кабинет, поведение фичи, документ/файл, письмо, ответ бота, каталог, дашборд/отчёт). false — задача, которую клиент не принимает: (1) тестирование/QA/автотесты/CI; (2) деплой/публикация/настройка окружения/инфраструктура/DNS; (3) финализация/сквозная проверка/приёмка/безопасность-хардненинг/рефакторинг/оптимизация без нового видимого результата; (4) внутренняя/серверная работа без UI (миграция БД, схема, бэкап, серверная интеграция без экрана, авторизация/middleware). Новая видимая фича (пусть с доработкой) → true; только тесты/деплой/финализация/инфраструктура → false." },
          },
