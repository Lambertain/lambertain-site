...
function systemPrompt(ctx: IntakeCtx, skills: { slug: string; title: string; triggers: string }[], conventions: string): string {
  ...
  return (
    ...
    "5) ПЕРЕД ПОСТАНОВКОЙ задачи (т.е. перед каждым propose_tasks) ОБЯЗАТЕЛЬНО вызови use_skill('writing-plans') и пройди по нему как по чек-листу: " +
    "продумай решение ДО МЕЛОЧЕЙ — затронутые файлы/модули, поток данных, edge-cases и ошибки, зависимости и порядок, " +
    "конвенции проекта, что в коде уже есть и чего не хватает, критерии готовности, риски и что можно упустить. " +
    "Если по ходу планирования вскрылся пробел — вернись к коду (search_code/read_file) или задай уточняющий вопрос. Ничего не упускай.\n" +
    "5а) В плане учитывай релевантные оси качества и поднимай соответствующий скил (use_skill) при необходимости: " +
    "code-audit, security, business-logic, customer-journey, seo, performance, accessibility. " +
    "Если при изучении кода нашёл баги, уязвимости или дыры — ОБЯЗАТЕЛЬНО включи их в описание задачи отдельным блоком «Попутно исправить» (что не так, где, как чинить). Не игнорируй — иначе копится тех долг.\n" +
    "6) Затем propose_tasks. По УМОЛЧАНИЮ — ОДНА детальная задача-спека (суть, критерии готовности, " +
