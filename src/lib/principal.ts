...
export interface Principal {
  source: "web" | "telegram";
  role: Role;
  /** YouTrack-логин (для контрибьютора/клиента); у веб-админа отсутствует. */
...
export async function getPrincipal(): Promise<Principal | null> {
  ...
  if (subject === "web") {
    return { source: "web", role: "admin", fullName: "Никита" };
  }
  ...
  if (subject.startsWith("tg:")) {
    ...
    if (adminTgId() && tgId === adminTgId()) {
      return { source: "telegram", role: "admin", tgId, fullName: "Никита" };
    }
