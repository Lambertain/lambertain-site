...
export function ChatModal({ projects, locale, isContributor, feedbackKey }: { projects: Proj[]; locale: Locale; isContributor?: boolean; feedbackKey?: string }) {
    ...
    <>
      ...
      {open && (
        <div style={{ position: "fixed", inset: 0, zIndex: 1000, background: "var(--bg)", display: "flex", flexDirection: "column" }}>
          ...
          <ChatIntake projects={projects} locale={locale} fill isContributor={isContributor} feedbackKey={feedbackKey} />
        </div>
      )}
    </>
  );
