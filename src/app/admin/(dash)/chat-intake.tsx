...
type Img = { id: string; mime: string; data: string; name: string };
...
/** Разбить тело (markdown с маркерами ![..](att:ID)) на блоки с сохранением порядка. */
function buildBlocks(text: string, images: Img[]): Block[] {
  const blocks: Block[] = [];
  const re = /!\[[^\]]*\]\(att:([^)]+)\)/g;
  let last = 0;
  ...
  while ((m = re.exec(text))) {
    ...
    const img = images.find((i) => i.id === m![1]);
    if (img) blocks.push({ type: "image", mime: img.mime, data: img.data });
    last = re.lastIndex;
  }
...
export function ChatIntake({ projects, locale, fill }: { projects: Proj[]; locale: Locale; fill?: boolean }) {
  ...
  function addFiles(files: FileList | File[] | null) {
    ...
    Array.from(files).forEach((f) => {
      if (!f.type.startsWith("image/")) return; // в тело — только картинки
      const reader = new FileReader();
      reader.onload = () => {
        const [meta, data] = String(reader.result).split(",");
        const mime = meta.slice(5, meta.indexOf(";"));
        const id = `i${++idSeq.current}`;
        const name = f.name || `screen-${id}`;
        setImages((p) => [...p, { id, mime, data, name }]);
        insertAtCursor(`\n![${name}](att:${id})\n`);
      };
