...
export async function POST(req: Request) {
  ...
  let body: { projectKey?: string; railwayToken?: string; projectId?: string; environmentId?: string; serviceId?: string; pgServiceId?: string };
  ...
  await setProjectMeta(projectKey, proj.name, { ...proj.meta, clientDeploy: cd });
  revalidatePath(`/admin/projects/${projectKey}`);

  const have = (k: string) => (cd as Record<string, string>)[k] ? "set" : "—";
  return NextResponse.json({
    ok: true, projectKey,
    clientDeploy: { railwayToken: have("railwayToken"), projectId: cd.projectId ?? null, environmentId: cd.environmentId ?? null, serviceId: cd.serviceId ?? null, pgServiceId: cd.pgServiceId ?? null },
  });
}
