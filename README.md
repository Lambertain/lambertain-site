...
- `/` — публичное портфолио
- `/admin` — PM-портал (задачи, проекты, команда, дашборд загрузки, скилы интейка, генерация договоров)
- `/tma` — точка входа Telegram Mini App
- `/api/webhooks/trello` — вебхук синка комментов с Trello-доски клиента (для проектов на Trello, напр. SadiControl)
