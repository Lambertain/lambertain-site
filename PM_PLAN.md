...
> **Статус: реализовано 2026-06-11** (детали — в PM_PORTAL.md). Правки относительно плана по ходу:
> - **Один разработчик на проект** (не массив `assignees`): связь = `meta.defaultAssignee`; «мульти-проекты» =
>   один дев ведёт несколько проектов. Назначение — на `/admin/team`.
> - **ИИ-ревью — on-demand, не авто** (поллер не гоняет токены): кнопка на странице задачи, вердикт — комментарием,
>   статус не меняет (рантайм разработчик проверяет сам на dev). Реализация — `src/lib/review.ts` (TS, не poller).
> - **Блокеры (зависимости задач)** — добавлены сверх плана: `task_deps`, авто-блок + запрет старта.
> - Валюта по умолчанию **₴**, прогресс-бар — **оба** (по времени и по задачам).
>
> Ниже — исходный план (для истории).
...
9. i18n добить, `npm run build` → 0 ошибок, коммит/пуш в main. **Деплой и миграция — автоматические по пушу**:
   Railway деплоит main, preDeployCommand web-сервиса (`node scripts/migrate.mjs`) прогоняет миграцию перед стартом.
